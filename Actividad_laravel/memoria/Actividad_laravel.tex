...
Para modificar esta configuración por defecto se debe modificar el fichero \configfile{app/Http/Controllers/Auth/LoginController.php} en el que añadiremos las variables \mintinline{php}{$decayMinutes} y \mintinline{php}{$maxAttempts} quedando como lo siguiente:
...
Tal como aparece en el mensaje, ha habido demasiados intentos de acceso fallidos y habrá que esperar 10795 segundos (los casi 180 minutos indicados en la configuración).
...
Los datos deben de validarse antes de acceder a la base de datos, y por tanto se debe de realizar en el \textit{\textbf{backend}} del portal.
...
En caso de que queramos añadir nuevos campos deberemos de modificar el fichero de la vista \configfile{resources/views/auth/register.blade.php} y añadir los campos que nos interese.

Modificaremos \configfile{app/Http/Controllers/Auth/RegisterController.php} para que estos campos posteriormente se inserten en la base de datos, dejando el fichero:
...
En las validaciones expuestas, se han realizado validaciones propias para los campos \textbf{dni}, \textbf{teléfono} e \textbf{IBAN}. Estas validaciones propias están configuradas dentro del directorio \configdir{app/rules}, donde cada fichero es una validación y contienen básicamente dos secciones:
