...
class Movie(private val movieService: MoviesInterface, private val moviesDataStore: DataStore<MovieStore>) {
    ...
    init {
        ...
        coroutineScope.launch {
            d { "rugolid: Initialize data store..." }
            moviesDataStore.data
                .map { it.initialized }
                .filter { !it }
                .first {
                    d { "rugolid: Initialize data store 2..." }
                    initDataStore()
                    return@first true
                }
        }
    ...
    private fun initDataStore() {
        ...
        val moviesFromJson: List<com.rugoli.moviedb.dataclass.Movie> = adapter.fromJson(Movie.MOVIES)!!

        // create the storedAnimals list
        val moviesToStore = moviesFromJson.map { it.asStoredMovie() }
        d{"rugolid: qweqwe" + moviesFromJson}
